...
def model_input(input_node_name, keep_prob_node_name):
    x = tf.placeholder(tf.float32, shape=[None, 224*224], name=input_node_name)
...
def build_model(x, keep_prob, y_, output_node_name):
    ...
    conv4 = tf.layers.conv2d(pool2, 512, 3, 1, 'same', activation=tf.nn.relu)
	
    pool4 = tf.layers.max_pooling2d(conv3, 2, 2, 'same')
...
def export_model(input_node_names, output_node_name):
    ...
    with tf.gfile.Open('out/frozen' + '.pb', "rb") as f:
    ...
    with tf.gfile.FastGFile('out/skin_graph'+ '.pb', "wb") as f:
